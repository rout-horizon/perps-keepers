...
export class LiquidationKeeper extends Keeper {
    ...
    async execute(): Promise<void> {
        try {
            const rpcprovider = new providers.JsonRpcProvider("https://sly-solitary-aura.arbitrum-sepolia.quiknode.pro/80e7211daaac0ba78007cdd7329ed52274d98322/");
            ...
            this.logger.info(`Found ${openPositions.length} open position(s) to check`);

            if (openPositions.length > 0) {
                const pageSize = 50 // MULTICALL_PAGE_SIZE;
                // let successfulOrders: Array<IORDER> = [];

                // Array to store promises for each callStatic result
                const canLiquidateStaticCallPromises: Promise<any>[] = [];
                const isFlaggedStaticCallPromises: Promise<any>[] = [];

                // Paginate the orders
                for (let i = 0; i < openPositions.length; i += pageSize) {
                    const paginatedOrders = openPositions.slice(i, i + pageSize);

                    const canLiquidateStaticCalls = paginatedOrders.map(order => {
                        return {
                            target: this.market.address,
                            callData: this.market.interface.encodeFunctionData("canLiquidate", [order.account]),
                            allowFailure: true,
                        }
                    })

                    const isFlaggedStaticCalls = paginatedOrders.map(order => {
                        return {
                            target: this.market.address,
                            callData: this.market.interface.encodeFunctionData("isFlagged", [order.account]),
                            allowFailure: true,
                        }
                    })

                    // Add the static call to the promises array
                    canLiquidateStaticCallPromises.push(multicall.callStatic.aggregate3(canLiquidateStaticCalls));
                    isFlaggedStaticCallPromises.push(multicall.callStatic.aggregate3(isFlaggedStaticCalls));
                }
                ...
                const canLiquidateStaticResultsArray: STATIC_CALL_RESULT[][] = await Promise.all(canLiquidateStaticCallPromises);
                const isFlaggedStaticResultsArray: STATIC_CALL_RESULT[][] = await Promise.all(isFlaggedStaticCallPromises);

                // Flatten the results from paginated calls
                const canLiquidateResults = canLiquidateStaticResultsArray.flat();
                const isFlaggedResults = isFlaggedStaticResultsArray.flat();

                // Filter positions that are not flagged and can be liquidated
                const filteredPositions = openPositions.filter((order, index) => {
                    const canLiquidateResult = canLiquidateResults[index];
                    const isFlaggedResult = isFlaggedResults[index];

                    // Decode return values from canLiquidate and isFlagged calls
                    const canLiquidate = canLiquidateResult.success && this.market.interface.decodeFunctionResult("canLiquidate", canLiquidateResult.returnData)[0];
                    const isFlagged = isFlaggedResult.success && this.market.interface.decodeFunctionResult("isFlagged", isFlaggedResult.returnData)[0];

                    // Return only positions that are not flagged and can be liquidated
                    return !isFlagged && canLiquidate;
                });

                // Handle filtered positions
                console.log("Filtered Positions:", filteredPositions);

                if (filteredPositions.length > 0) {
                    for (let index = 0; index < filteredPositions.length; index++) {
                        const element = filteredPositions[index];
